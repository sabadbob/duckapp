...
export function TodayTab() {
  ...
  const prompt = doneCount === total
    ? 'Everything ticked. I have nothing to hold over you. Go to sleep.'
    : "No membership, no excuse — push day is a floor and twelve reps. Are we doing it or admiring the floor?";
    ...
    <TabScroll>
      ...
      <div style={{ padding: '14px 16px 16px', borderBottom: '2px solid var(--ink)', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, paddingLeft: 39 }}>
          <button className="btn2 btn2o" style={{ padding: '11px 12px' }} onClick={() => s.didIt()}>Did it</button>
          <button className="btn2" style={{ padding: '11px 12px' }} onClick={() => s.cantDo()}>Can't today</button>
        </div>
        {s.log.map((r, i) => (
      ...
      <div style={{ padding: '0 16px' }}>
        {s.ledger.map((it, i) => (
          <button key={i} className="row" onClick={() => s.toggleLedgerItem(i)}>
            ...
            <div style={{ flex: 1, font: "400 15px/1.25 'Source Serif 4',serif", color: it.done ? '#8f8b85' : '#232323', textDecoration: it.done ? 'line-through' : 'none' }}>{it.label}</div>
            <div style={{ font: "600 9.5px/1.5 'Archivo'", letterSpacing: '.1em', color: it.done ? '#7d7a74' : (it.meta === 'DUE' || it.meta.includes('PIN-UP') ? '#9a4d00' : '#4d545e'), fontVariantNumeric: 'tabular-nums' }}>{it.meta}</div>
          </button>
        ))}
      </div>

      <div style={{ marginTop: 12, borderTop: '2px solid var(--ink)', background: 'var(--panel)', padding: '13px 16px 16px' }}>
